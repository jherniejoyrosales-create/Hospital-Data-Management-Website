...
import { ZBenefitPackage } from '../lib/types';
import { toast } from 'sonner';
import { zBenefitPackages } from '../lib/zBenefitPackages';

export default function ZBenefitsPage() {
  const [packages, setPackages] = useState<ZBenefitPackage[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [selectedPackage, setSelectedPackage] = useState<ZBenefitPackage | null>(null);
  ...
  const loadPackages = async () => {
    ...
    const stored = localStorage.getItem('hospitalZBenefits');
    if (stored) {
      try {
        setPackages(JSON.parse(stored));
      } catch (e) {
        console.error('Error parsing Z Benefits:', e);
        setPackages(zBenefitPackages);
        localStorage.setItem('hospitalZBenefits', JSON.stringify(zBenefitPackages));
      }
    } else {
      // First load - use default packages
      setPackages(zBenefitPackages);
      localStorage.setItem('hospitalZBenefits', JSON.stringify(zBenefitPackages));
      toast.success(`Successfully loaded ${zBenefitPackages.length} Z Benefit packages!`);
    }
    setLoading(false);
  };
  ...
  const handleEditPackage = (pkg: ZBenefitPackage) => {
    setSelectedPackage(pkg);
    setIsEditDialogOpen(true);
  };

  const handleDeletePackage = async (pkg: ZBenefitPackage) => {
    if (confirm(`Are you sure you want to delete Z Benefit Package ${pkg.code}?`)) {
      const updatedPackages = packages.filter(p => p.code !== pkg.code);
      setPackages(updatedPackages);
      localStorage.setItem('hospitalZBenefits', JSON.stringify(updatedPackages));
      toast.success(`Z Benefit Package ${pkg.code} deleted successfully!`);
    }
  };

  const handleSavePackage = async (pkg: ZBenefitPackage) => {
    let updatedPackages: ZBenefitPackage[];
    
    if (selectedPackage) {
      // Update existing
      updatedPackages = packages.map(p => p.code === selectedPackage.code ? pkg : p);
      toast.success(`Z Benefit Package ${pkg.code} updated successfully!`);
    } else {
      // Add new
      ...
      toast.success(`Z Benefit Package ${pkg.code} added successfully!`);
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-[#0D47A1] text-white p-6 rounded-t-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Z Benefit Packages</h1>
              <p className="text-blue-100 mt-1">PhilHealth special benefit packages for high-cost medical interventions</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-b-lg overflow-hidden shadow-sm">
          {loading ? (
            <div className="p-12 text-center text-gray-500">
              ...
              <p className="mt-4">Loading Z Benefit packages...</p>
            </div>
          ) : filteredPackages.length === 0 ? (
            <div className="p-12 text-center text-gray-500">
              <Package className="w-16 h-16 mx-auto mb-4 text-gray-300" />
              <p className="text-lg">No Z Benefit packages found</p>
        ...
        {!loading && (
          <div className="mt-4 text-center text-sm text-gray-600">
            Showing {filteredPackages.length} of {packages.length} Z Benefit packages
          </div>
        )}
      </div>
...
interface PackageDialogProps {
  pkg: ZBenefitPackage | null;
  onSave: (pkg: ZBenefitPackage) => void;
  onClose: () => void;
}

function PackageDialog({ pkg, onSave, onClose }: PackageDialogProps) {
  const [formData, setFormData] = useState<ZBenefitPackage>({
    code: pkg?.code || '',
  ...
  const handleChange = (field: keyof ZBenefitPackage, value: string | number) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-[#0D47A1] text-white p-6 rounded-t-lg flex justify-between items-center sticky top-0 z-10">
          <h2 className="text-xl font-bold">
            {pkg ? 'Edit Z Benefit Package' : 'Add New Z Benefit Package'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Enter detailed description of the Z Benefit package"
              rows={3}
              className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#2196F3] focus:border-transparent ${
